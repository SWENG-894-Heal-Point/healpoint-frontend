...
const SignupPage = (props) => {
    ...
    function handleSubmission(values) {
        ...
        axios.get("/user-existence", {params: {email: userInfo.email}})
            .then((response) => {
                // add logic to log the user in from successful signup
        ...
        <Formik
            ...
            {({ values, handleChange }) => (
                <AccountWrapper url="/login" urlText="Log in" propmtText="Have an account?&nbsp;" >   
                    ...
                    <Field id="password" name="password" placeholder="Password" type={isShowPassword ? "text" : "password"} required
                        onChange={(e) => {
                        handleChange(e);
                        setPasswordsMatch(e.target.value === values.confirmPassword);
                    }}
                    />
                    <span id={style.show_password} onClick={() => setShowPassword(!isShowPassword)}>{isShowPassword ? "Hide" : "Show"}</span>
                    <Field id="confirmPassword" name="confirmPassword" placeholder="Confirm Password" type={isShowPassword ? "text" : "password"} required onChange={(e) => {
                        handleChange(e);
                    ...
                    <div className={style.error}>
                        ...
                        { values.password.length > 0 && values.confirmPassword.length > 0 && !isPasswordMatch && <p>Passwords do not match</p>}
                    </div>
                    }
